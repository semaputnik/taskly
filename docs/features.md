...
- **FR-01.27** A user can merge tags: every task carrying the merged tags
  ...
  after being told how many tasks change. A merge is one activity log entry and
  cannot be restored. Renaming into a taken name stays refused.
- **FR-01.28** The Tags page offers groups of tags whose names differ only in
  letter case, separators, whitespace or a trailing plural, for merging. A group
  ...
  them instead. A tag a bot user created names that bot user.
